...
#        imgblack=imgblack[0:BLACK_IMAGE_WIDTH_HEIGHT,0:BLACK_IMAGE_WIDTH_HEIGHT] # cropping black image
        ...
        for contourWithData in validContoursWithData:            # for each contour
            ...
            imgROI = imgGray[contourWithData.intRectY : contourWithData.intRectY + contourWithData.intRectHeight,     # crop char out of threshold image
...
filename='Convolutional_Neural_network_model.h5'

case=input("Do you want to retrain the model?(for first time input 'y')[y/n]")
if(case=='y' or case=='Y'):
    ##Importing datasets -----------------------------------------------------------------------------------------------------------------------------------
    
    # Getting local classification dataset------------------------------------------------------------------------------------------------------------------
    try:
        npaClassifications = np.loadtxt("classifications_gray.txt", np.float32)                  # read in training classifications
    except:
        print ("error, unable to open classifications_gray.txt, exiting program\n")
        os.system("pause")
    try:
        npaFlattenedImages = np.loadtxt("flattened_images_gray.txt", np.float32)                 # read in training images
    except:
        print ("error, unable to open flattened_images_gray.txt, exiting program\n")
        os.system("pause")
...
#    npaFlattenedImages = sc_npaFlattenedImages.fit_transform(npaFlattenedImages)
    ...
    images2D=[]
    # Reshape flattened gray images to 30x20 2D array and store in list
    for npaFlattenedImage in npaFlattenedImages:
        images2D.append(npaFlattenedImage.reshape(RESIZED_IMAGE_HEIGHT,RESIZED_IMAGE_WIDTH,1))
    images2D=np.array(images2D)# converting list to 3D numpy array
    
    # Attempt to format into keras.preprocessing.image.DirectoryIterator-------
    
    #--------------------------------------------------------------------------
    
    # Initialising CLASSIFIERS for convolutional neural network---------------------------------------------------------------------------------------
    '''
    ...
    # Step 1 - Convolution
    classifier_cnn.add(Convolution2D(16,                                        # number of filters       
                                     2, 2,                                      # shape of filter
                                     input_shape = (RESIZED_IMAGE_HEIGHT,RESIZED_IMAGE_WIDTH,1) ,                  # shape of input image
                                     activation = 'relu'))                      # Rectifiler activation for convolution layer
    
    # Step 2 - Pooling
    classifier_cnn.add(MaxPooling2D(pool_size = (2, 2)))                        # Max-pooling
    
    # Adding a second convolutional layer
    classifier_cnn.add(Convolution2D(16, 2, 2, activation = 'relu'))
    classifier_cnn.add(MaxPooling2D(pool_size = (2, 2)))
                                                     
    # Step 3 -  Re-Flattening
    classifier_cnn.add(Flatten())

    # Step 4 - Full connection
    classifier_cnn.add(Dense(output_dim = 128, activation = 'relu'))
    classifier_cnn.add(Dense(output_dim = len(npaClassifications[0]), activation = 'softmax'))
    
    # Compiling the CNN
    classifier_cnn.compile(optimizer = 'adam', loss = 'categorical_crossentropy', metrics = ['accuracy'])
    
    from sklearn.utils import shuffle
    images2D,npaClassifications=shuffle(images2D,npaClassifications)
    
    datagen = ImageDataGenerator(
                featurewise_center=True,
                featurewise_std_normalization=True,
                rotation_range=20,                                              # can be 45, 60, 90
#                width_shift_range=0.2,
#                height_shift_range=0.2,
#                horizontal_flip=True                                           #Horizontal flip not required
                                )
    
    # compute quantities required for featurewise normalization
    # (std, mean, and principal components if ZCA whitening is applied)
    datagen.fit(images2D)
    
    # fits the model on batches with real-time data augmentation: i.e flipping and rotating image
    classifier_cnn.fit_generator(datagen.flow(images2D, npaClassifications, batch_size=32),
                        steps_per_epoch=len(images2D) / 32, epochs=120)
    
    # Fitting CNN
#    classifier_cnn.fit(images2D, npaClassifications,
...
classifier_cnn = load_model(filename)

for test_image_number in range(15,16):
...
#        print(npaROIResized)
        npaROIResized=npaROIResized.reshape(1,RESIZED_IMAGE_HEIGHT,RESIZED_IMAGE_WIDTH,1)
        y_pred = classifier_cnn.predict(npaROIResized)
